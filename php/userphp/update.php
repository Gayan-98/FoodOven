...
if(isset($_GET["id"])){
    $ID = $_GET["id"];
    $sql = "SELECT * FROM `add_recipe` WHERE `Recipe_ID` = $ID";
    $res = $con->query($sql);
    if($res -> num_rows > 0)
    {   
        $row = $res->fetch_assoc();
...
<label class="formele">ID :</label><br><br>
 ...
 -User name :<br></br>
   <input class="us" type="text" placeholder="Add your name" name="user_name" value= "<?php echo $uname ?>"></br></br>
 -Title :</br></br>
   <input class="e" type="text" placeholder="Recipe title" name="Title" value= "<?php echo $title ?>"></br></br>
 -Cooking Time :</br></br>
   <input class="w" type="text" placeholder="Time Duration" name="Time" value= "<?php echo $time ?>"></br></br>
 -Main ingredients :</br></br>
   <textarea class="ing" rows="10" cols="35" name="Ingredients" ><?php echo $ing ?></textarea></br></br>
 -Description :</br></br>
   <textarea class="des" rows="8" cols="50" name="Description" ><?php echo $des ?></textarea></br></br>
...
if(isset($_POST['submit'])){
  ...
  $query="UPDATE add_recipe SET Recipe_ID='$ID',category='$category',user_name='$username',Title='$Title',Time='$Time',Ingredients='$Ingredients',Description='$Description',Images='$Images' WHERE Recipe_ID = '$ID' ";  
